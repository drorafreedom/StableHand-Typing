...
function imageExists(url: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    img.onload = () => resolve(true);
    img.onerror = () => resolve(false);
    // only cache-bust in dev
    const q = import.meta.env.DEV ? ((url.includes('?') ? '&' : '?') + 'v=' + Date.now()) : '';
    img.src = url + q;
  });
}
...
const discoverPublicUrlsFast = async (): Promise<string[]> => {
  const base = (import.meta as any).env?.BASE_URL || '/';
  const exts = ['jpg', 'jpeg', 'png', 'webp', 'gif', 'bmp'];
  const urls: string[] = [];
 ...
 */
 ...
 // inside useEffect that seeds images (runs once)
useEffect(() => {
  if (settings.urls.length) return;

  // try 1.* immediately, in parallel
  (async () => {
    const base = ((import.meta as any).env?.BASE_URL || '/') + 'bgphotos/1';
    const exts = ['jpg','jpeg','png','webp'];
    const candidates = exts.map(ext => `${base}.${ext}`);

    const results = await Promise.all(candidates.map(imageExists));
    const first = candidates.find((_, i) => results[i]);

    if (first) {
      // show one immediately
      setSettings(s => ({ ...s, urls: [first] }));
      setIdx(0); setRunning(true);
    }


// drop-in replacement (or add alongside as discoverPublicUrlsFast)
async function discoverPublicUrlsFast(maxIndex = 200): Promise<string[]> {
  const base = (import.meta as any).env?.BASE_URL || '/';
  const exts = ['jpg','jpeg','png','webp','gif','bmp'];

  const exists = imageExists; // use your function from #1

  const urls: string[] = [];
  const BATCH = 16;          // concurrency
  const MAX_CONSEC_MISSES = 4;

  let i = 1;
  let misses = 0;

  while (i <= maxIndex) {
    const batchIdx = Array.from({ length: BATCH }, (_, k) => i + k).filter(n => n <= maxIndex);

    // Make one URL per index (first ext that exists)
    const tasks = batchIdx.map(async n => {
      for (const ext of exts) {
        const u = `${base}bgphotos/${n}.${ext}`;
    ...
    for (const u of found) {
      if (u) { urls.push(u); anyHit = true; }
    }

    misses = anyHit ? 0 : (misses + 1);
    if (urls.length > 0 && misses >= MAX_CONSEC_MISSES) break;
  ...
  return urls;
    ...
    // then load the full list (see #3) and replace when ready
    const urls = await discoverPublicUrlsFast();
    if (urls.length) setSettings(s => ({ ...s, urls }));
  })();
  // eslint-disable-next-line react-hooks/exhaustive-deps
}, []);
