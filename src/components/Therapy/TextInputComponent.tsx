// src/components/TherapyPage/TextInputComponent.tsx
...
interface AnimationParams {
  [key: string]: string | number | string[] | number[]; // Adjust types based on actual structure
}
...
const TextInputComponent: React.FC<TextInputComponentProps> = ({ animationParams }) => {
      ...
      const csvData = Object.keys(formDataWithTimestamp)
        .map((key) => {
          const value = Array.isArray(formDataWithTimestamp[key])
            ? (formDataWithTimestamp[key] as (string | number)[]).join(';')
