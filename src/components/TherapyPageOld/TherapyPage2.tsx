...
const TherapyPage2 = () => {
  ...
  const [patternType, setPatternType] = useState('sineWave');
    ...
    <body>
        ...
        <script>
            ...
            function recordKeystroke(event) {
                if (event.type === 'keydown'){
                    ...
                    if (lastTimestamp != null){
                        keystroke.pressToPress = keystroke.pTimestamp - lastTimestamp;
                    }
            ...
            function toCSV(keystrokes){
                let csvFile = 'keystroke,pTimestamp,rTimestamp, holdTime,pressToPress\n';
                for (const keystroke of keystrokes) {
                    const rTimestamp = keystroke.rTimestamp ? new Date (keystroke.rTimestamp).toISOString() : 'null';
                    // const holdTime = keystroke.holdTime !== undefined ? keystroke.holdTime : 'null';
                    // const pressToPress = keystroke.presstoPress !== undefined ? keystroke.pressToPress : 'null';
                    csvFile += `${keystroke.key},${new Date(keystroke.pTimestamp).toISOString()},${rTimestamp || ''},${keystroke.holdTime || ''},${keystroke.pressToPress || ''}\n`;
                    //csvFile += `${keystroke.key},${new Date(keystroke.pTimestamp).toISOString()},${rTimestamp},${holdTime},${pressToPress}\n`
                }
                return csvFile;
            }
            ...
            function makeResizable(element, initialWidth, initialHeight) {
                ...
                function resizeMouseDown(e) {
                    e = e || window.event;
                    e.preventDefault();
                    let startX = e.clientX + initialWidth;
                    let startY = e.clientY + initialWidth;
            ...
            function displayRandomWords() {
                ...
                makeResizable(rectangle, initialWidth, initialHeight);
                // compareCharacters();
            ...
            function handleTextInput() {
                ...
                makeResizable(textInputWrapper, initialWidth, initialHeight);
            }
            ...
            function compareCharacters(){
                const charTyped = allKeystrokes // modify to work on pieces of text
                    .map(keystroke => keystroke.key)
                    .join('')
                const charDisplay = rectangle.innerText; // text from dictionary

                let correct = 0; // counters
                let error = 0;
                const min = Math.min(charTyped.length, charDisplay.length); // smaller length 

                for (let i = 0; i < min; i++){
                    if (charTyped[i] === charDisplay[i]) {
                        correct++;
                    } else if (charTyped[i] !== ' ') {
                        error++;
                    }
                ...
                const totalTypedCharacters = charTyped.replace(/ /g, '').length;
                const totalDisplayCharacters = charDisplay.replace(/ /g, '').length;

                const accuracy = (correct / totalDisplayCharacters) * 100;
                const errors = error;
            ...
            document
                .getElementById("stop")
                .addEventListener("click", speedStop)
